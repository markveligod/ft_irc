...
void			IRC::create_socket()
{
	if ((this->client = socket(AF_INET, SOCK_STREAM, 0)) < 0)
		IRC::error("Esteblishing socket error");
...
bool IRC::end_connection(const char *line)
{
	std::string str(line);
...
void IRC::error(std::string const &str)
{
